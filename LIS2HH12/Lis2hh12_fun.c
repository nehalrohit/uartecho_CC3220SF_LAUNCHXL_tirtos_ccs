...
//}
//
//float getActiveTime() { return movingTime; }
//
//float getActiveTime2() { return movingTime10; }
//
...
void setReboot() { writeRegister(128, LIS2HH12_CTRL6, 127); }
...
void setSoftReset() { writeRegister(64, LIS2HH12_CTRL5, 191); }
...
uint8_t readRegister(uint8_t reg)
{
    ...
    uint8_t bufp=0;
#if i2c_handle
    HAL_I2C_Mem_Read(LIS2HH12_I2C_ADD_L, reg ,&bufp, 1);
#endif
        ...
        uint8_t txBuffer[2];
        uint8_t rxBuffer[2];
        memset(txBuffer,NULL,sizeof(txBuffer));
...
void writeRegister(uint8_t val, uint8_t regis, uint8_t bt) {
  ...
  // that you want read and change to zero
    ...
    val2 = result | val;
#if i2c_handle
    HAL_I2C_Mem_Write(LIS2HH12_I2C_ADD_L, regis ,&val2, 1);
 #endif

#ifdef spi_handle
  platform_write(regis ,&val2, 1);
#endif

}
int32_t platform_write(uint8_t reg_addr, uint8_t *reg_data, uint16_t len)
{
int index;
